...
import maintenanceFakerFactory from "./maintenanceFakerFactory";
...
const MaintenanceFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = maintenanceFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("maintenance").create(elem));
...
export default connect(mapState, mapDispatch)(MaintenanceFakerDialogComponent);
